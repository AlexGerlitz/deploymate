...
if [ "$SURFACE" = "frontend" ] || [ "$SURFACE" = "full" ]; then
    ...
    if [ "$DEPLOYMATE_FRONTEND_FAST_MODE" = "skip" ]; then
      ...
      for frontend_smoke in "${frontend_fast_smokes[@]}"; do
        case "$frontend_smoke" in
          auth|beginner|ops|review|runtime|servers)
            ;;
          *)
  ...
  if [ "$FAST_MODE" != "1" ]; then
    ...
    run_frontend_smokes_shared "$shared_full_port" "/tmp/deploymate-frontend-full-smoke.log" ".next-smoke-full-${shared_full_port}" \
      auth ops review beginner runtime admin admin-interactions servers templates
...
if [ "$SURFACE" = "frontend" ] || [ "$SURFACE" = "full" ]; then
  if [ "$FAST_MODE" = "1" ]; then
    if [ "$DEPLOYMATE_FRONTEND_FAST_MODE" = "skip" ]; then
      ...
      echo "[release]   - frontend preflight plus auth, ops, review, and runtime smokes"
    fi
  else
